...
export const AdhkarReader: React.FC = () => {
  ...
  const adhkarList = categoryId && adhkarData[categoryId] ? adhkarData[categoryId] : [];
  
  const [counts, setCounts] = useState<Record<number, number>>(
    adhkarList.reduce((acc, dhikr) => ({ ...acc, [dhikr.id]: dhikr.count }), {})
  );
  ...
  const isCompleted = (id: number) => counts[id] === 0;
  const allCompleted = adhkarList.every(d => isCompleted(d.id));
